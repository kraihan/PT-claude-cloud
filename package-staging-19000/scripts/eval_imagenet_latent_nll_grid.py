...
def _plot(summary: list[dict], output: Path) -> None:
    ...
    for inner in inner_budgets:
        ...
        outer = np.asarray([int(r["K_outer"]) for r in rows])
        nll = np.asarray([float(r["nll_per_dim_nats_mean"]) for r in rows])
        ci = np.asarray([float(r["nll_per_dim_nats_ci95"]) if np.isfinite(float(r["nll_per_dim_nats_ci95"])) else 0.0 for r in rows])
        ax.errorbar(outer, nll, yerr=ci, marker="o", capsize=3, label=f"K_inner={inner}")
    ax.set_xscale("log", base=2)
    ax.set_xlabel("outer Monte-Carlo budget K_outer")
    ax.set_ylabel("latent NLL / dimension (nats; lower is not certified tighter)")
    ax.set_title("Fixed ImageNet-1k validation latents; 95% MC intervals")
    ax.grid(alpha=0.25)
...
def main() -> None:
    ...
    _plot(summary, out / "nll_budget_grid.png")
