...
def pt_potential_step(
    ...
    *,
    ...
    generator_mean = m0
    recovery_metrics = {}
    if sched.policy == "active_recovery_v1":
        from ptflow.recovery import refine_proposal
        m0, recovery_metrics = refine_proposal(
            pot, x0, m0, c0, steps=sched.proposal_refine_steps, lr=sched.proposal_refine_lr,
        )
    s0 = None
    ...
    # Report the unrefined generator proposal separately. Good ESS following
    # local refinement does not imply the one-pass generator is well matched.
    if sched.policy == "active_recovery_v1" and (sched.step == 0 or (sched.step + 1) % 20 == 0):
        from ptflow.estimator import tilted_phi0
        with torch.no_grad():
            raw_s = unwrap_ddp(scale_net)(generator_mean, c0) if scale_net is not None else None
            raw_est = tilted_phi0(pot, x0, c0, generator_mean, raw_s, eps,
                                 K=K, alpha_def=sched.alpha_def(), generator=rng, chunk=chunk)
            metrics["pt/generator_proposal_ess"] = raw_est.ess.mean()
            metrics["pt/generator_proposal_control_ess"] = ((raw_est.ess - 1.0 / K) / (1.0 - 1.0 / K)).clamp(0, 1).mean()
    if not update:
    ...
    if sched.policy == "active_recovery_v1" and sched.alignment_lambda() > 0:
        from ptflow.recovery import alignment_loss
        n_align = min(B, sched.alignment_batch)
        # Match class-conditioned pretrained transport; unconditional dropout
        # is retained in the likelihood/scale branches above.
        la = alignment_loss(pot, x0[:n_align], generator_mean[:n_align], labels_noise[:n_align])
        loss = loss + sched.alignment_lambda() * la
        metrics["pt/loss_alignment"] = la.detach()
    if not torch.isfinite(loss):
        raise FloatingPointError("Nonfinite potential/scale/alignment loss")
    loss.backward()
    ...
    potential_opt.step()
    metrics["pt/potential_updated"] = torch.ones((), device=device)
    metrics["pt/scale_updated"] = torch.as_tensor(float(scale_net is not None and sched.lambda_scale > 0), device=device)
