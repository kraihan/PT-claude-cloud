...
class PTSchedule:
    ...
    # Explicit experimental recovery policy. Legacy runs retain ESS gating.
    # This schedules progress by optimizer iterations, not a health certificate.
    policy: str = "ess_gated"
    recovery_check_after: int = 1500
    recovery_bad_patience: int = 500
    proposal_refine_steps: int = 8
    proposal_refine_lr: float = 0.5
    alignment_weight: float = 0.1
    alignment_hold: int = 1000
    alignment_end: int = 5000
    ...
    def state_dict(self) -> Dict[str, float]:
        return {
            ...
            "policy": self.policy,
            "measured_healthy_steps": self.measured_healthy_steps,
    ...
    def load_state_dict(self, d: Dict) -> None:
        if not d:
            return
        saved_policy = d.get("policy", "ess_gated")
        if saved_policy != self.policy:
            raise ValueError(f"Schedule policy mismatch: {saved_policy} -> {self.policy}; start a fresh run")
        ...
        self.measured_healthy_steps = int(d.get("measured_healthy_steps", 0))
        self.consecutive_bad = int(d.get("consecutive_bad", 0))
    ...
    def update_potential(self) -> bool:
        ...
        if self.policy == "active_recovery_v1":
            return True
    ...
    def observe(self, ess: float | None) -> str:
        ...
        if self.policy == "active_recovery_v1":
            if ess is None or not math.isfinite(float(ess)):
                raise FloatingPointError("Active PT requires a finite measured control ESS every iteration")
            state = self.health.update(ess)
            self.measured_healthy_steps += int(state == HEALTHY)
            self.step += 1
            self.eps_progress = self.step
            self.prox_progress = min(1.0, max(0.0, (self.step - self.prox_warmup) / max(1, self.prox_ramp)))
            if self.step >= self.recovery_check_after and state == BROKEN:
    ...
    def recovery_failure(self) -> str | None:
        if self.policy == "active_recovery_v1" and self.consecutive_bad >= self.recovery_bad_patience:
            return (f"Active PT estimator remained broken for {self.consecutive_bad} iterations; "
                    f"control ESS EMA={self.health.value:.6g}. Checkpoint saved; refusing a silent 30k failure.")
        return None

    def alignment_lambda(self) -> float:
        if self.policy != "active_recovery_v1":
            return 0.0
        t = min(1.0, max(0.0, (self.step - self.alignment_hold) / max(1, self.alignment_end - self.alignment_hold)))
        return self.alignment_weight * (1.0 - t)
    ...
    def metrics(self, device=None) -> Dict[str, torch.Tensor]:
        vals = {
            ...
            "pt/active_policy": float(self.policy == "active_recovery_v1"),
            "pt/measured_healthy_steps": float(self.measured_healthy_steps),
            "pt/consecutive_bad": float(self.consecutive_bad),
            "pt/alignment_weight": self.alignment_lambda(),
        }
...
def build_schedule(cfg: Dict | None) -> PTSchedule:
    ...
    if sched.policy not in {"ess_gated", "active_recovery_v1"}:
        raise ValueError(f"Unknown schedule policy: {sched.policy}")
    if health_cfg:
