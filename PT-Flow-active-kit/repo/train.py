...
def train_gen(
    ...
    if init_ema_from and local_ckpts:
        # The first launch initializes EMA weights at step zero. A requeue
        # restores this isolated run's complete state instead of reinitializing.
        log_for_0("Existing checkpoints: resuming full state; not reloading initializer")
        init_ema_from = ""
    if resume_from and local_ckpts:
    ...
    if init_ema_from:
        ...
        initialize_ema_only(state, init_ema_from)
    ...
    for step in pbar:
        ...
        if state.pt is not None and state.pt.sched.policy == "active_recovery_v1":
            from ptflow.run_guard import inspect_boundary
            inspect_boundary(state, metrics, workdir)
