...
class WandbLogger:
    ...
    def set_logging(
        ...
        **kwargs,
        ...
        # Local metrics are also the source for offline CSV reports when W&B is on.
        resolved_offline_dir = workdir_path / "log" if workdir_path is not None else Path(offline_dir)
        ...
        if self.use_wandb:
            ...
            if workdir_path is not None:
                init_kwargs["dir"] = str(workdir_path)
            if allow_resume:
    ...
    def _flush_buffer(self) -> None:
        ...
        for k, v in reduced.items():
            if torch.is_tensor(v):
                tensor_groups.setdefault(v.device, []).append((k, v))
    ...
    def log_dict(self, d: Dict[str, Any]) -> None:
        ...
        for k, v in d.items():
            ...
            if isinstance(v, (int, float, np.floating, np.integer)):
                reduced[k] = float(v)
        for k, v in reduced.items():
            self._buffer[k] = self._buffer.get(k, 0.0) + v
