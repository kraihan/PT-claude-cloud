...
# Slurm --export uses commas as variable separators.  Preserve the user-facing
# comma syntax outside Slurm, then send a separator that cannot split export.
EPS_LIST_SLURM="${EPS_LIST//,/;}"
...
source ~/ptflow_env.sh
export WORK="${WORK:-/scratch/$USER/ptflow}"
export REPO="${PTFLOW_PROX_REPO:-${REPO:-$WORK/PT-Flow}}"
...
SBATCH_ARGS=(
  ...
  --export=ALL,PTFLOW_PROX_REPO="$REPO",PTFLOW_PROX_RESULTS="$OUT/results",PTFLOW_PROX_EPS="$EPS_LIST_SLURM",PTFLOW_PROX_CFG="$CFG_SCALE",PTFLOW_PROX_POINTS="$POINTS",PTFLOW_PROX_POINT_BATCH="$POINT_BATCH",PTFLOW_PROX_K="$K_REF",PTFLOW_PROX_CHUNK="$PROPOSAL_CHUNK",PTFLOW_PROX_MC_SEEDS="$MC_SEEDS",PTFLOW_PROX_ALPHA="$ALPHA_DEF"
)
