...
does not generate FID samples, train, or alter a checkpoint.  It writes:
  ...
  raw.csv / summary.csv
  proposal_components.svg
  epsilon_sweep.svg
  proposal_mismatch.svg
...
def _plot_components(summary: list[dict], out: Path, eps: float) -> None:
    ...
    panels = []
    for metric, ylabel in (("ess_over_k", "ESS / K (higher is better)"),
                           ("log1p_chi2", "log(1 + chi2_hat) (lower is better)")):
        panels.append({
            "kind": "bar", "xlabels": labels, "xlabel": "proposal", "ylabel": ylabel,
            "series": [{"label": "mean +/- 95% MC CI", "x": list(range(len(rows))),
                        "y": [float(row[f"{metric}_mean"]) for row in rows],
                        "err": [float(row[f"{metric}_ci95"]) for row in rows], "color": "#2774a6"}],
        })
    render_panels_svg(out, title=f"Proposal components at epsilon={eps:g}; fixed ImageNet-latent x0/labels",
                      panels=panels, columns=2)
...
def _plot_epsilon(summary: list[dict], out: Path) -> None:
    ...
    for metric, ylabel in (("ess_over_k", "ESS / K (higher is better)"),
        ...
        for proposal in PROPOSALS:
            ...
            series.append({"label": PROPOSAL_LABELS[proposal], "x": [float(r["epsilon"]) for r in rows],
                           "y": [float(r[f"{metric}_mean"]) for r in rows],
                           "err": [float(r[f"{metric}_ci95"]) for r in rows], "color": COLORS[proposal]})
        panels.append({"kind": "line", "x_log": True, "series": series, "xlabel": "epsilon", "ylabel": ylabel})
    render_panels_svg(out, title="Estimator health versus epsilon; same fixed x0/labels across the sweep",
                      panels=panels, columns=2)


def _plot_mismatch(summary: list[dict], out: Path) -> None:
    panels = []
    for axis_name, xlab in (("center", "center shift beta"), ("scale", "std. multiplier")):
        for metric, ylabel in (("ess_over_k", "ESS / K"), ("log1p_chi2", "log(1 + chi2_hat)")):
            series = []
            for proposal in ("learned_diagonal", "full_defensive"):
                ...
                series.append({"label": PROPOSAL_LABELS[proposal], "x": [float(r["mismatch_level"]) for r in rows],
                               "y": [float(r[f"{metric}_mean"]) for r in rows],
                               "err": [float(r[f"{metric}_ci95"]) for r in rows], "color": COLORS[proposal]})
            panels.append({"kind": "line", "title": f"{axis_name} mismatch", "series": series,
                           "xlabel": xlab, "ylabel": ylabel})
    render_panels_svg(out, title="Proposal-mismatch stress test at the checkpoint epsilon", panels=panels, columns=2)
...
def main() -> None:
    ...
    _plot_components(summary_rows, out / "proposal_components.svg", component_eps)
    _plot_epsilon(summary_rows, out / "epsilon_sweep.svg")
    _plot_mismatch(summary_rows, out / "proposal_mismatch.svg")
