...
source ~/ptflow_env.sh
export WORK="${WORK:-/scratch/$USER/ptflow}"
export REPO="${PTFLOW_NLL_REPO:-${REPO:-$WORK/PT-Flow}}"
...
ARGS=(--parsable --partition="$PARTITION" --nodes=1 --ntasks=1 --cpus-per-task="$CPUS" --mem="$MEMORY" --time="$WALLTIME" --gpus="$GPU_TYPE:1" --job-name=ptf-latent-nll-19k --output="$OUT/logs/%x_%j.out" --error="$OUT/logs/%x_%j.err" --export=ALL,PTFLOW_NLL_REPO="$REPO",PTFLOW_NLL_RESULTS="$OUT/results",PTFLOW_NLL_CACHE="$CACHE",PTFLOW_NLL_POINTS="$POINTS",PTFLOW_NLL_SEEDS="$MC_SEEDS")
